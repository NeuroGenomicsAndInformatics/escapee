...
bcftools view \
	-i 'GT[*]="RA"' \
	escapee-subtracted.denovo.vcf.gz \
	| bgzip -c > escapee.denovo.het.vcf.gz	
	
# switch chr form to 1..22 X Y for pipeline
bcftools annotate \
	--rename-chrs chr_change.txt \
	escapee.denovo.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.vcf.gz

# SnpEff annotation for predicted impact
snpeff \
	-v GRCh38.105 \
	escapee.denovo.het.chrs.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.vcf.gz

# dbNSFP annotation
snpsift dbnsfp \
	-db dbNSFP4.1a.txt.gz \
	-v escapee.denovo.het.chrs.snpeff.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.vcf.gz

# dbSNP annotation
snpsift annotate \
	-id dbSNP_b153_GRCh38.gz \
	-v escapee.denovo.het.chrs.snpeff.dbnsfp.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.vcf.gz
...
bcftools annotate \
	...
	-h CADD_hdr.txt \
	escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.vcf.gz
...
bcftools annotate \
	...
	-h revel_hdr.txt \
	escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.vcf.gz

# gnomAD annotation
# Need to change chromosome format back to chr1,chr2,chrX
bcftools annotate \
	--rename-chrs chr_changeback.txt \
	escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.chrs.vcf.gz

for CHR in chr{1..22} chrX chrY; do
	bcftools view \
	-r $CHR \
	escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.chrs.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.${CHR}.vcf.gz
done
...
parallel -j 24 \
	...
	escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.{}.vcf.gz \
	| bgzip -c > escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.gnomad.{}.vcf.gz" \
	::: chr{1..22} chrX chrY
...
gatk MergeVcfs \
	${VCFS[@]} \
	-O escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.gnomad.ALLchr.vcf.gz

# make a readable text file
zcat escapee.denovo.het.chrs.snpeff.dbnsfp.dbsnp.CADD.REVEL.gnomad.ALLchr.vcf.gz \
